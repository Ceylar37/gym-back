...
export type RequestWithBody<Request extends NextRequest, Schema extends z.ZodSchema> = Request & {
  getBody: () => z.infer<Schema>;
};

export const withBody = <Schema extends z.ZodSchema>(schema: Schema) => {
  return <Request extends NextRequest>(cb: (req: RequestWithBody<Request, Schema>) => Promise<NextResponse>) => {
